...
func consecutiveAnalysisCodes(config *model.Alarm)  {
	...
	if (config.Start >0 && config.End >0) && (time.Now().Hour() < config.Start || time.Now().Hour() > config.End)  {
		...
		logger.Log("时时彩-2连1站报警 报警通知非接受时间段内")
		return
	}
	...
	*/
	...
	go func(config *model.Alarm) {
		...
		if q3_num == config.Number || z3_num == config.Number || h3_num == config.Number {
			//发送邮件
			mail.SendMail(CpTypeName[CqsscType] + " 2连1站", body)
		}
	}(config)
	...
	go func(config *model.Alarm) {
		...
		if q3_num >= config.Number {
			body += "<div> 彩种: " + CpTypeName[TjsscType] + " 2连1站报警提示 位置: 前三 期数: "+ strconv.Itoa(q3_num) + "</div>"
		}
		if z3_num >= config.Number {
			body += "<div> 彩种: " + CpTypeName[TjsscType] + " 2连1站报警提示 位置: 中三 期数: "+ strconv.Itoa(z3_num) + "</div>"
		}
		if h3_num >= config.Number {
			body += "<div> 彩种: " + CpTypeName[TjsscType] + " 2连1站报警提示 位置: 后三 期数: "+ strconv.Itoa(h3_num) + "</div>"
		}
		body += q3_log_html
		body += z3_log_html
		body += h3_log_html
		...
		if q3_num >= config.Number || z3_num >= config.Number || h3_num >= config.Number {
			//发送邮件
			mail.SendMail(CpTypeName[TjsscType] + " 2连1站", body)
		}
	}(config)
	*/
	...
	go func(config *model.Alarm) {
		...
		if q3_num == config.Number || z3_num == config.Number || h3_num == config.Number {
			//发送邮件
			mail.SendMail(CpTypeName[XjsscType] + " 2连1站", body)
		}
	}(config)
	...
	go func(config *model.Alarm) {
		...
		if q3_num >= config.Number {
			body += "<div> 彩种: " + CpTypeName[TwsscType] + " 2连1站报警提示 位置: 前三 期数: "+ strconv.Itoa(q3_num) + "</div>"
		}
		if z3_num >= config.Number {
			body += "<div> 彩种: " + CpTypeName[TwsscType] + " 2连1站报警提示 位置: 中三 期数: "+ strconv.Itoa(z3_num) + "</div>"
		}
		if h3_num >= config.Number {
			body += "<div> 彩种: " + CpTypeName[TwsscType] + " 2连1站报警提示 位置: 后三 期数: "+ strconv.Itoa(h3_num) + "</div>"
		}
		body += q3_log_html
		body += z3_log_html
		body += h3_log_html
		...
		if q3_num >= config.Number || z3_num >= config.Number || h3_num >= config.Number {
			//发送邮件
			mail.SendMail(CpTypeName[TwsscType] + " 2连1站", body)
		}
	}(config)
	*/
...
func getCqCodes() ([]string, []string, []string) {
	...
	if len := len(consecutive_cq_data); len >0 {
		...
		if new_code == newcode {
			log.Println("时时彩-2连1站 彩票类型:", CpTypeName[CqsscType], "已经分析过了,等待新的开奖号码出现...")
			return q3s, z3s, h3s
		}
...
func getTjCodes() ([]string, []string, []string) {
	...
	if len := len(consecutive_tj_data); len >0 {
		...
		if new_code == newcode {
			log.Println("时时彩-2连1站 彩票类型:", CpTypeName[TjsscType], "已经分析过了,等待新的开奖号码出现...")
			return q3s, z3s, h3s
		}
...
func getXjCodes() ([]string, []string, []string) {
	...
	if len := len(consecutive_xj_data); len >0 {
		...
		if new_code == newcode {
			log.Println("时时彩-2连1站 彩票类型:", CpTypeName[XjsscType], "已经分析过了,等待新的开奖号码出现...")
			return q3s, z3s, h3s
		}
...
func getTwCodes() ([]string, []string, []string) {
	...
	if len := len(consecutive_tw_data); len >0 {
		...
		if new_code == newcode {
			log.Println("时时彩-2连1站 彩票类型:", CpTypeName[TwsscType], "已经分析过了,等待新的开奖号码出现...")
			return q3s, z3s, h3s
		}
